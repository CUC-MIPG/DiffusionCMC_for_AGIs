...
class BasicCMC:
    ...
    def init_models(self):
        ...
        # layer1
        if self.args.custom_prompts is None and self.args.text_src is None:
        ...
        # layer2
        if args.edgeq_src is None:
            self.edge_detector = PidiNetDetector.from_pretrained(
                'lllyasviel/Annotators'
                ).to(self.args.stable_diffusion_device)

        self.edge_adapter = T2IAdapter.from_pretrained(
        ...
        self.t2i_pipeline_layer3 = StableDiffusionAdapterPipeline(
            ...
            tokenizer=self.ldm.tokenizer,
            unet=self.ldm.unet,
            adapter=MultiAdapter([self.edge_adapter,self.color_adapter]),
            scheduler=self.ldm.scheduler,
            safety_checker=self.ldm.safety_checker,
            feature_extractor=self.ldm.feature_extractor
            )
    ...
    def edge_detection_pidi(self, ref_image):
        edge_image = self.edge_detector(ref_image,apply_filter=True)
        return edge_image
    ...
    def text_to_image_with_multiple_guidance(self, text, image):
        assert len(image) > 1, 'More than one guidance required'
...
if __name__ == '__main__':  
    ...
    parser.add_argument('--edgeq_src','-e',default=None,help='custom edge detection images, won\'t save the edge images if provided')
    ...
    parser.add_argument('--vectorize','-v',action='store_true',help='Vectorize the edge image and save as svg (bad performance)')
    ...
    for image in os.listdir(args.image_src):
        ...
        if args.custom_prompts is not None:
            ...
            try:
                with open(os.path.join(args.text_src,image.replace('jpg','txt').replace('png','txt'))) as f:
        ...
        # extract/open edge
        if args.edgeq_src is not None:
            edge_image = Image.open(os.path.join(args.edgeq_src,image.replace('jpg','png')))
        else:
            edge_image = processor.edge_detection_pidi(ref_img)
            
        # Alternative way to compress edge
        if args.vectorize and args.edgeq_src is None:
            os.makedirs(os.path.join(edge_outdir,"origin"))
            os.makedirs(os.path.join(edge_outdir,"vector"))
            os.makedirs(os.path.join(edge_outdir,"reconstructed"))
            edge_image.save(os.path.join(edge_outdir,"origin",image))

            # convert to svg and save
            processor.write_svg(edge_image, os.path.join(edge_outdir,"vector",image.split('.')[0]))

            # generate bitmap from svg
            cairosvg.svg2png(url=f"{os.path.join(edge_outdir,'vector',image.split('.')[0])}.svg", write_to=f"{os.path.join(edge_outdir,'reconstructed',image.replace('jpg','png'))}")
            edge_bitmap = Image.open(f"{os.path.join(edge_outdir,'reconstructed',image.replace('jpg','png'))}").convert('L')
            
            if args.edgeq_src is None:
                edge_image.save(os.path.join(edge_outdir,image))

        else:
            edge_bitmap = edge_image.convert('L').resize((512,512))
        ...
        print('Processing: ',image,' with caption: ',image_caption)
        ...
        # gen layer2
        if dl_list[1]:
            layer2_recon = processor.text_to_image_with_guidance(image_caption, edge_bitmap)
            layer2_recon.save(os.path.join(layer2_outdir,image))

        # gen layer3
        if dl_list[2]:
            layer3_recon = processor.text_to_image_with_multiple_guidance(image_caption, [edge_bitmap, color_image])
            layer3_recon.save(os.path.join(layer3_outdir,image))

        # saving text 
        if args.text_src is None and args.custom_prompts is None:
            with open(os.path.join(text_outdir,image.replace('jpg','txt').replace('png','txt')),'w') as f:
